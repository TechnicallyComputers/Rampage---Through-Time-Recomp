...
#
# Writes: dist/rtt-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix rtt \
  --exe-name Rampage___Through_Time__Recompiled \
  --display-name "Rampage - Through Time  Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Rampage - Through Time disc" \
